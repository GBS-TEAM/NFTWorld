...
// const compressVideo = (sourceUri: any, new_uri: string) => {
//   console.log('  compressVideo  ++++', sourceUri);
//   FFmpegKit.execute(`-i ${sourceUri} -c:v mpeg4 file2.mp4`).then(
//     (result) => {
//       console.log('>>>>>>>>>>>', result);
//     },
//   );
      ...
      selected.map(async (t, idx) => {
        ...
        if (image.playableDuration) {
          try {
            ...
            let newSource =  `${RNFS.DocumentDirectoryPath}/video.zscaled_new${rndm_name}.mp4`;
            let fullcommand = `-i ${videoUri}  -c:v libx264 -preset ultrafast ${str_for_ffmpeg_command} -c:a copy  ${newSource}`;
            // let fullcommand = `-i ${videoUri}  -c:v libx264 -preset faster  ${str_for_ffmpeg_command} -c:a copy  ${newSource}`;
